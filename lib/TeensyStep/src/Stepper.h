...
namespace TeensyStep
{
    ...
    public:
        inline int32_t getMaxSpeed() const { return vMax; }
    ...
    protected:
        ...
        int32_t A, B; // Bresenham parameters
        int32_t vMax;
    ...
    void Stepper::doStep()
    {
        GPIOManager::writePin(stepPin, polarity ? GPIO_PIN_SET : GPIO_PIN_RESET);
        current += dir;
    }

    void Stepper::clearStepPin() const
    {
        GPIOManager::writePin(stepPin, polarity ? GPIO_PIN_RESET : GPIO_PIN_SET);
    }

    void Stepper::setDir(int d)
    {
        dir = d;
        GPIOManager::writePin(dirPin, (dir == 1) ? (reverse ? GPIO_PIN_SET : GPIO_PIN_RESET)
