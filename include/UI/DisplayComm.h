...
#include <Arduino.h>
#include "c/LumenProtocol.h" // Corrected include path for Lumen Protocol
...
// typedef void (*ButtonHandler)(uint8_t button_id); // Old handler
using PacketHandler = void (*)(const lumen_packet_t *packet); // New handler for Lumen packets
...
public:
    ...
    void updateText(uint8_t id, bool value); // Added overload for boolean

    // Button handling
    // void setButtonHandler(ButtonHandler handler); // Old setter
    void setPacketHandler(PacketHandler handler); // New setter
...
private:
    ...
    // ButtonHandler _buttonHandler; // Old handler member
    PacketHandler _packetHandler; // New handler member

    // Private methods
    void sendCommand(const char *cmd); // This will be updated to use Lumen
    // void handleRxData(uint8_t data); // This will be removed
};
